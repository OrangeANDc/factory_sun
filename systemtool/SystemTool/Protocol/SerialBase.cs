...
namespace SystemTool.Protocol
{
    public class SerialBase
    {
        ...
        public bool ReadData(byte[] address, ushort length, ref byte[] result)
        {
            try
            {
                if (!_serialPort.IsOpen)
        ...
        public bool ReadData_MicroDevice(byte[] address, ushort length, ref byte[] result)
        {
            try
            {
                if (!_serialPort.IsOpen)
        ...
        public bool WriteMulData_MicroDevice(byte[] address, byte[] data)
        {
            try
            {
                if (!_serialPort.IsOpen)
        ...
        private bool SendAndReceiveFlashHead_Datalog(byte[] sendData)
        {
            try
            {
                ...
                {
                    Thread.Sleep(100);
                    if (Environment.TickCount - tickCountLast > 8000)
                    {
                        Log.Debug("recv : " + Modbus.ByteArrayToSting(_receiveData));
                        Log.Debug("数据接收长度错误/超时");
                        return false;
